...
// TestProxyAgentEndpoint_AbsentDepthStartsAtZero verifies a top-level /tasks hop
// carrying no X-Ephemera-Task-Depth header is treated as depth 0 and forwarded to
// the agent as depth 1 (0+1) — the "absent depth starts at 0" contract.
func TestProxyAgentEndpoint_AbsentDepthStartsAtZero(t *testing.T) {
	var gotDepth string
	var seen bool
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
	...
	// No X-Ephemera-Task-Depth header on the incoming request.
	r := httptest.NewRequest(http.MethodPost, "/vms/vm-1/tasks", strings.NewReader(`{"prompt":"x"}`))
	w := httptest.NewRecorder()
	cp.proxyAgentEndpoint(w, r, "vm-1", "/tasks")
	if !seen {
	...
	if gotDepth != "1" {
		t.Errorf("expected forwarded X-Ephemera-Task-Depth=1 (absent depth 0, +1), got %q", gotDepth)
	}
}

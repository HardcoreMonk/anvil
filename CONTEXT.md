...
| flock guest 능력 토큰 | flock member VM의 `/root/.ephemera-cp-token`에 주입되는 per-flock secret. **두 flock kind가 같은 모델을 쓴다** — routed flock은 adapter가 발급한 `relay_token`, local flock은 daemon이 flock 생성 시 발급한 토큰이며, 둘 다 같은 admission store(`cp.relayTokens`)에 등록돼 그 flock의 wall sub-path와 `call` 진입만 admit한다. 운영자 control-plane bearer를 guest에 주입하지 않는다. local flock 토큰은 flock 디렉토리의 `guest-token`(0600)에 영속되고 daemon 시작 시 재수화되며, 만료가 없어 **폐기 수단은 flock 삭제뿐**이다. 결정: [ADR-0003](docs/adr/0003-per-flock-guest-capability-tokens.md) | `cmd/goose-daemon`, `internal/orchestrator` |

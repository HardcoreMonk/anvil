...
**In-VM control-plane token (v0.3.3, hot-rotated v0.3.4, narrowed by ADR-0003)** — The host injects a **per-flock guest capability token** into each local flock VM at `/root/.ephemera-cp-token` (mode 0600) via `spawnVMOptions.ControlPlaneToken`, and sets `ControlPlaneTokenManaged: false` at all three member spawn sites (`spawnVMForFlock`, `restartAgent`, `changeFlockAgentRole` in `cmd/goose-daemon/orchestrator_api.go`). The token is minted lazily by `cp.ensureLocalFlockGuestToken`, registered in the same relay-token admission store routed flocks use, persisted at `flocks/<id>/guest-token`, and rehydrated at startup — see [ADR-0003](docs/adr/0003-per-flock-guest-capability-tokens.md) for the design and `docs/guides/security-and-resilience.md` for the operator-facing rules. The daemon's own operator bearer (`controlPlaneTokenForVM`) is deliberately **not** wired into any spawn path; do not re-wire it. Standalone `POST /vms` injects nothing when the request carries no `control_plane_token` (`injectVMFiles` skips the file for an empty token, so a plain workload VM has no CP bearer on disk); when the caller does supply one it is written verbatim and left unmanaged — this is how a routed flock member gets its scoped per-flock relay token (`internal/anvilmcp/routed_flock.go`). v0.3.4's SIGHUP `SET_CP_TOKEN` fan-out is restricted to VMs whose token the daemon itself injected (`runningVM.cpTokenManaged`, see CP token vsock fan-out below); since no current spawn path sets that bit, its only remaining targets are VMs that predate ADR-0003 and still carry `cp_token_managed` in `state.json`, and the set drains as they are replaced. Keep `ControlPlaneTokenManaged: false` on every flock spawn site you touch — flipping it back would let a rotation overwrite a scoped capability token with the broad operator bearer.

**CP token vsock fan-out (v0.3.4)** — `ReloadClients` snapshots `cp.vms` under `cp.mu.RLock()` (NOT `clientsMu`), then fans `vm.SetGuestCPToken` calls out as parallel goroutines. Per-VM failures MUST be logged, not propagated — a stuck VM cannot block the SIGHUP path. The ~4 s per-VM budget (20 attempts × 200 ms, `vsockSendCommand` in `internal/vm/machine.go`) bounds each goroutine, and the parallel dispatch keeps total wall-clock at roughly that rather than 4 s × `len(vms)`. If you add new mutable VM-level state that needs to survive rotation, mirror this pattern: snapshot under `cp.mu.RLock()`, dispatch in parallel, swallow per-VM errors.
...
**First-non-expired CP token + post-loop expiry check (v0.4.1)** — `controlPlaneTokenForVM` and `propagateCPTokenToVMs` select the first NON-EXPIRED client (via `firstActiveClient`), not blindly `clients[0]`, so an expired primary token does not break the in-VM `/townwall/post` forwarder of a VM the daemon injected that bearer into; when every token has expired they propagate `""` (unauthenticated) and log a warning. (Since ADR-0003 that VM set is only pre-ADR-0003 spawns — a current flock member's forwarder uses its flock capability token and is unaffected by client expiry.) Mirror this if you add a new consumer of the "primary" token. Token expiry is checked in `authMiddleware` AFTER the constant-time compare loop (never inside it) so it adds no timing signal, and an expired match returns the same 401 body as an unknown token.

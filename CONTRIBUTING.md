...
**Firecracker SDK signal forwarding (v0.3.4 hot-fix, narrowed v0.4.0)** — `firecracker-go-sdk` v1.0.0's `firecracker.Config.ForwardSignals` defaults to `[SIGINT, SIGQUIT, SIGTERM, SIGHUP, SIGABRT]` and installs a goroutine (`setupSignals` in the SDK) that forwards each received signal to the Firecracker child via `cmd.Process.Signal(sig)`. `internal/vm/machine.go` defines a package-level `forwardSignals` slice — now just `[SIGQUIT, SIGABRT]` — passed to both `StartMachine` and `RestoreMachine`'s `firecracker.Config`. Three signals are deliberately omitted: **`SIGHUP`** (v0.3.4) because the daemon uses it for token hot reload and Firecracker has no SIGHUP handler — forwarding it would kill every running VM mid-rotation; **`SIGINT` and `SIGTERM`** (v0.4.0) because the daemon traps them itself (main.go) and runs a graceful teardown that explicitly `StopVMM`s each child (which sends its own SIGTERM), so forwarding is redundant — and v0.4.0 auto-snapshot runs a `Pause`+`CreateSnapshot` on each VM inside that `DestroyAll` window, which a forwarded SIGTERM would race and kill mid-snapshot. `SIGQUIT`/`SIGABRT` stay forwarded precisely because the daemon does NOT trap them, so on an abnormal exit (deferred teardown never runs) forwarding is what prevents orphaned Firecracker children. If you add new SDK call sites (a third spawn path, a re-attach path, etc.), set `ForwardSignals: forwardSignals` there too — leaving it nil silently re-enables all three bugs. The invariant: never add `SIGHUP` back while reload semantics are tied to it, and never add `SIGINT`/`SIGTERM` back while the daemon owns graceful exit (auto-snapshot depends on owning that window).

**Metrics counter discipline (v0.3.5)** — `internal/metrics/` is a hand-written exposition formatter; the registry validates metric and label names against `[a-zA-Z_:][a-zA-Z0-9_:]*` at registration time and panics on a duplicate or malformed name. When adding a counter, keep label sets closed and small (`outcome=ok|fail`, or `ok|denied|expired` for `ephemera_auth_total` — never free text); when adding a `type` label, document the closed set in the README catalogue. The `orchestrator/` package intentionally does NOT import `internal/metrics/`; the daemon wires watchdog observations via the `OnDead` / `OnHeal` / `OnProbeDuration` callbacks set on `Watchdog`. Keeping that direction one-way prevents a package cycle when watchdog tests run with no daemon-level registry.
...
**MCP stdio backend lifecycle (v0.6.4, `internal/mcpgateway/backend_stdio.go`)** — A stdio backend's subprocess must end up reaped on EVERY death path: the reaper goroutine (`reap`) is the sole `cmd.Wait` caller and the only place that fails pending calls, detaches the proc, and closes `dead`; kill initiations (`terminate`) only ever SIGKILL the **process group** while the leader is alive (which pins the pgid — never group-kill after `Wait`, the pgid may be recycled to an innocent process). Spawning is allowed only inside `ensureStarted`'s cooldown gate; a bypass reintroduces unbounded crash-loops. **A caller's ctx expiring must never kill the child** — it abandons only that pending wait (the spawn/handshake runs on its own 60s clock), otherwise a 5s health probe racing a slow first start (npx-style installs) produces a kill/respawn livelock that never converges. `Registry.Close()` must stay wired in `stopMCPGateway` AFTER the listener drain (kill children only when no VM call is in flight). `Pdeathsig` is insurance, not the mechanism — it fires on the death of the *thread* that spawned the child (go.dev/issue/27505), which is fine for whole-daemon death but nothing else. `RLIMIT_NPROC` is applied only on the privilege-drop path: it counts every process of the uid system-wide, so clamping a shared uid (non-root daemon) can kill the child at startup because of unrelated load — that exact failure took out the whole unit-test suite during v0.6.4 development. `RLIMIT_AS` is deliberately absent (V8/JVM/sanitizer children die at startup under any useful cap); do not "harden" it back in. The scratch dir (child cwd + HOME) must keep a chain the **de-privileged child can traverse**: base `/var/lib/ephemera/mcp-stdio` (NOT under the work dir — that commonly sits below a 0700/0750 home the stdio user cannot enter), created parents 0755 (`ensureScratchDir` chmods them explicitly; a plain `MkdirAll(dir, 0700)` propagates 0700 to parents), leaf 0700. Go's fork/exec runs `cmd.Dir`'s chdir AFTER setuid in the child, so a traversal failure surfaces as a misleading `fork/exec <command>: permission denied` — this broke e2e step 87 during v0.6.4 development.

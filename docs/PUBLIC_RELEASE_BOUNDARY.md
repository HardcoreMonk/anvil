...
| `v0.4.2` | COW spawn probe/fallback, COW+Diff snapshot | `adapted`, default plain 확정(**D4 종결, upstream-tracked**) — `EPHEMERA_DISK_MODE=cow`는 명시적 opt-in, default plain 무기한 유지. burn-in에서 D4(diff-restore guest GPF)가 부하 하 재발, 4라운드 조사로 anvil-측 소진·근본은 anvil 밖 KVM/fc resume-race로 확정 → 2026-07-13 종결(fc v1.16.1이 실패율 100%→~25%로 최대 완화). flip 재개는 upstream 해소 시에만(상세 `docs/ADR_INDEX.md` v0.4.2 행 + `operations/2026-07-11-cow-burnin-run.md` round 1~4 + `operations/2026-07-13-d4-firecracker-upstream-report.md`) |

...
| `v0.4.2` | COW spawn probe/fallback, COW+Diff snapshot | `adapted`, default cow deferred(**D4 미해결로 보류**) — `EPHEMERA_DISK_MODE=cow`는 명시적 opt-in. PR #46 fix 후 green은 flip 재검증에서 D4 재발로 n=1 우연이었음이 드러남; 재개는 fc 업그레이드/anvil 완화가 host-a·host-b n≥2 green 통과 후(상세 `docs/ADR_INDEX.md` v0.4.2 행 + `operations/2026-07-11-cow-burnin-run.md`) |

...
| Flock guest 능력 토큰 | flock member VM의 `/root/.ephemera-cp-token`에 주입되는 자격증명은 **per-flock guest 능력 토큰**이며, local·routed 두 kind가 같은 모델을 공유한다(routed는 adapter 발급 `relay_token`, local은 daemon이 flock 생성 시 발급). 두 경우 모두 같은 admission store에 등록돼 `authMiddleware`가 경로에서 뽑은 flock id와 exact match할 때만 admit되므로, 범위는 **그 flock의 wall sub-path(`post\|wall\|wall/history`) + `call` 진입**뿐이다 — 다른 flock의 같은 경로도, `/vms`·`/config/*`·`/tenants`·`/snapshots` 같은 control-plane 경로도 열지 않는다. 운영자 control-plane bearer는 guest에 주입하지 않는다. local flock 토큰은 flock 디렉토리의 `guest-token`(명시 0600, atomic tmp+rename)에 영속되고 daemon 시작 시 재수화되며, `metadata.json`에는 어떤 토큰도 기록되지 않는다(무조건 불변식, 테스트 강제). **만료·개별 회전 없음 — 폐기는 flock 삭제뿐**이며(삭제 시 admission + 토큰 파일 동시 제거), 이 수명 거래는 ADR-0003의 잔여 위험 계약에 명시돼 있다. SIGHUP 운영자 토큰 회전은 능력 토큰을 덮어쓰지 않는다(능력 토큰 이전에 spawn된 VM만 회전 대상으로 남아 교체와 함께 소진). guest 측 코드 변경 없음, 새 `anvil_*` MCP tool 없음 | `cmd/goose-daemon/flock_guest_token.go`, `cmd/goose-daemon/api.go`(authMiddleware relay-token admission), `internal/orchestrator/guest_token.go`, `docs/adr/0003-per-flock-guest-capability-tokens.md` |
...
| Cross-host snapshot replication | 스냅샷을 다른 host로 복제해 host 단일 장애점을 줄인다. **수동 경로**(baseline, 2026-06-02): `anvil_replicate_snapshot` MCP tool이 명령형 escape hatch로 source→target export/import 스트림을 한 번 연결한다(현행 유지, 상태는 자동 경로와 `SnapshotLocations`를 공유). **자동 경로**(이 slice): adapter reconcile 루프가 스냅샷마다 desired replica factor(**상수 N=2**, 원본+복제 1)로 **best-effort eventual 수렴**한다 — 스냅샷 생성이 복제본 완료를 기다리지 않으며 **무손실 보장이 아니다**. dial 실패는 연속 3회(상수 cap) 뒤 giving-up, 대상 복귀 시 재시도 재개. D3 coarse-fs 거부/tenant 불일치는 terminal(비재시도, 프로세스 수명 한정 — 재시작 시 re-arm). 복제본 GC/전파는 비목표(스냅샷 삭제가 다른 replica에 전파되지 않음). **전제:** adapter↔runtime host daemon은 control-plane 포트로 상호 도달 가능한 신뢰(private) 네트워크 위에 있어야 한다 — sweep의 discovery 단계는 각 peer daemon의 `ListSnapshots` 응답을 replica 위치로 받아들이므로, 위 Town Wall 행과 같은 신뢰 경계가 이 경로에도 배포 요구사항으로 적용된다. 외부 노출은 기존 reverse-proxy/TLS 정책 뒤에서만. **위치 출처(provenance)**: 각 위치는 `verified`(adapter가 직접 배치·확인 — replicate/import 성공 또는 `CreateSnapshot`)와 `observed`(peer daemon 보고)로 구분해 기록되며, `verified`는 이후 `observed` 보고로 강등되지 않는다(provenance map이 없는 기존 state 파일의 위치는 둘 중 어느 쪽도 아닌 unknown). **새 metric 표면**: scheduler `/metrics`에 `anvil_scheduler_snapshot_replication_*`(attempts_total, latency_seconds, queue_depth, giving_up, peer_only_satisfied, last_success/last_failure timestamp)를 노출한다 — label은 저-cardinality `outcome`/`reason`/`phase`뿐이며 host 주소·토큰·snapshot id는 어떤 label/log에도 없다(공개 안전). anvil 경계는 metric 노출 + runbook 권장 alert 식 문서화까지이며 in-adapter alerting/notification은 없다(실 alerting은 zone 대시보드 몫). runtime/operator surface이며 새 `anvil_*` MCP tool을 추가하지 않는다. 유닛(`-race`) + KVM e2e `scripts/anvil-snapshot-replication-e2e.sh`(대상 down→giving-up→복귀→자동 복제→metrics 전이, 2연속 green)로 검증됐다 | `internal/anvilmcp/snapshot_replication.go`, `internal/anvilmcp/runtime_router.go`, `internal/anvilmcp/placement_store.go`, `internal/anvilmcp/scheduler_metrics.go`, `docs/superpowers/specs/2026-07-11-snapshot-replication-automation-design.md` |
| Scheduler tenant quota metrics | scheduler `/metrics`에 fleet-summed 테넌트 quota 사용률을 노출한다: `anvil_scheduler_quota_usage_total{resource}` / `anvil_scheduler_quota_limit_total{resource}`(resource ∈ `snapshot_bytes, snapshot_count, active_vms, concurrent_tasks, retained_audit_records`), `anvil_scheduler_quota_tenants_near{resource}` / `_tenants_over{resource}`(near/over 상호 배타, near 임계 0.9 고정), `anvil_scheduler_quota_tenants_total`(라벨 없음). **aggregate-only 계약**: 어떤 quota metric도 `tenant_id`/host 라벨을 넣지 않는다(무인증 `/metrics` + bounded-enum 규율) — "어느 테넌트가 near/over인지"는 metric에 없고, host의 quota store JSON을 직접 조회해야 한다(운영 절차는 `docs/operations/runbook.md`). runtime/operator surface이며 새 `anvil_*` MCP tool을 추가하지 않는다 | `internal/anvilmcp/scheduler_metrics.go`, `docs/operations/observability.md`, `docs/operations/runbook.md` |
...
| multi-host/scheduler/quota | runtime 안정성, 보안 경계, 운영 문서, full KVM E2E 기준이 먼저 정리되어야 한다. (aggregate quota **metrics**는 §2 "Scheduler tenant quota metrics" 행으로 이미 졸업했다 — 운영 문서(observability.md/runbook.md) 요건 충족. 이 조건부 행은 quota API/config 등 나머지 표면에만 적용된다.) |

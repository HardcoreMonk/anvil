...
| **복구 중 transient egress 창**(신규 — 구현 세부에서 확인) | VM 복구는 per-VM egress를 `registerRecoveredVM`의 마지막(VM 부팅 + agent 응답 후) 단계에서 재적용한다(spawn은 부팅 **전** 적용이라 이 창이 없다). 호스트 리부트(iptables 전소) 후엔 복구 VM의 부팅~agent-wait 구간 동안 per-VM 규칙이 아직 없어 base 서브넷 blanket ACCEPT만 걸린다(일시 fail-open). 데몬 재시작 시엔 이전 규칙이 persist하나 `flushEgressByComment`→`applyEgressPolicy` 사이에 마이크로 갭이 있다. 재적용 후 durable 종상태는 항상 fail-closed(실패 시 fence REJECT) | 창은 **일시적·self-healing**이며, 이전에 호스트 리부트가 **영구** fail-open이던 것을 봉쇄한 개선이다. 악용에는 적대적 in-guest 루트 + 정밀 타이밍이 필요 → 신뢰 워크로드 위협 모델 밖. 완전 제거에는 복구 경로에서도 spawn처럼 부팅 전 egress 적용이 필요(Follow-Up) |

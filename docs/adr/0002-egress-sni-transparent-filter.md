...
|---|---|---|
| **ECH/ESNI**(SNI 암호화) | anvil은 ClientHello의 **outer(공개/cover) SNI만** 관측한다(SNI 파싱은 server_name만 추출하고 ECH `encrypted_client_hello` 확장 `0xfe0d`는 스킵하나, **존재 자체는 best-effort 탐지 — 관측용, 아래 잔여 cell**). outer가 없거나 비허용 decoy면 allowlisted SNI가 없어 fail-closed DROP. **단 outer가 allowlisted 도메인(ECH 공개 이름)이면 flow는 허용되고 암호화된 inner 목적지는 은닉된다** — guest-asserted SNI와 동일 신뢰등급 잔여(허용 outer로 임의 inner 터널) | anvil은 ECH를 무력화하지 않는다(inner는 서버 키 없이 복호 불가). 완화는 해당 엔드포인트 **CIDR fallback 핀**뿐(OQ7, outer SNI는 신뢰 근거 아님). **ECH-on-allowed-flow는 이제 관측 가능(2026-07-18)**: 파서가 `0xfe0d`를 best-effort 탐지해 allowlisted outer로 허용된 flow가 ECH를 담으면 `ephemera_egress_sni_ech_observed_total{proto}` metric + content-free `slog.Info`를 방출한다(**verdict 불변 — deny 안 함**, SNI 파싱 의미도 불변). blanket ECH-deny는 채택하지 않는다(신뢰-워크로드 모델상 net-negative: allowlisted CDN의 정상 ECH 파손 · 모델 밖 위협 · 불완전). 상세는 spec `docs/superpowers/specs/2026-07-18-ech-observability-design.md` |
...
| **pre-decision 부분 ClientHello 전달**(신규 — 구현 세부에서 확인) | 멀티세그먼트 ClientHello에서, 아직 완결되지 않은 세그먼트는 SNI 판정이 나기 **전에** unmarked ACCEPT로 통과한다(`sniVerdictLoop.Start`의 `!done` 분기) — 다음 세그먼트가 같은 큐로 재진입해 판정을 이어간다. 완결/malformed 시점에만 최종 verdict(ACCEPT+mark 또는 DROP)가 나며, 그때만 metric/audit이 기록된다. 재조립 테이블(`sniReassemblerMaxFlows=4096`, per-flow `sni.maxClientHelloBytes=16384`)이 가득 차면 LRU eviction이 발생하고, evict된 흐름의 다음 세그먼트는 **새 reassembler 인스턴스**로 시작해 레코드 경계를 못 잡고 fail-closed DROP한다(never fail-open) | 이 전달은 **승인 누수가 아니다** — conntrack mark(fast-path 자격)는 오직 완결된 ClientHello의 positive SNI 매치에서만 찍힌다. 다만 16 KiB는 흐름당 하드 캡이 아니다: eviction 후 새 인스턴스가 시작되므로, 공격자가 세그먼트를 의도적으로 지연시키며 계속 "미완결" 상태를 유지하면 판정 전 unmarked 세그먼트가 반복 통과할 수 있다(각 세그먼트가 개별 NFQUEUE 판정을 거쳐야 하므로 TCP 자체의 전송 hiccup으로만 자연 rate-limit된다). 완전 봉쇄에는 판정 전 전송을 보류하는 hold-then-decide 재설계가 필요하다 — v1은 채택하지 않는다(YAGNI). **재검토 결과(2026-07-18): 기각 확정.** 잔여는 무해하다 — 전달되는 건 TLS ClientHello 앞부분 바이트뿐이고, deny 시 완결 세그먼트가 DROP+RST돼 서버는 전체 ClientHello를 못 받아 핸드셰이크가 완결되지 않는다(응답 경로 없음·부분 데이터라 exfil 불가·승인 누수 없음). 반면 hold-then-decide는 미완결 세그먼트를 drop해야 하는데(NFQUEUE는 ACCEPT/DROP만 가능), 그러면 클라의 TCP 재전송이 같은 바이트를 다시 Feed해 현재의 단순 byte-append reassembler(`sni.Reassembler`)를 오염시켜 정상 핸드셰이크를 파손한다 — 즉 TCP reassembler를 seq-offset 인지(중복 dedup·재정렬)로 재작성(QUIC `InitialReassembler` 수준)해야 하고, 모든 멀티-세그먼트(PQ ~1.5KB) TCP 핸드셰이크에 +1 RTO(~200ms) 지연을 더한다. QUIC의 connmark race 같은 강제 요인도 없어(TCP CH은 flow 첫 패킷이 아니라 conntrack이 핸드셰이크로 선-confirm됨) 순수 비용이다. 무해한 잔여를 위해 재작성+지연을 감수할 근거가 없다. **2026-08-06 갱신 (감사 H3)**: 이 기각 논증은 *완결 세그먼트가 언젠가 도달해 최종 verdict가 난다*를 숨은 전제로 삼는데, 그 전제가 실제로는 성립하지 않고 있었다 — `ParseClientHelloSNI`가 첫 TLS 레코드의 길이 필드에 파스를 고정해, RFC 8446 §5.1이 허용하는 **멀티-레코드 ClientHello는 영원히 미완결**로 남아 `!done` 분기를 무기한 반복했다(fail-OPEN, metric·audit 0건). 레코드 평탄화 수정으로 전제가 복원됐고, 이제 이 행의 논증은 건전하다. 두 가지 사실도 함께 정정한다: (a) `!done` 분기는 이제 `ephemera_egress_sni_verdict_total{outcome="incomplete"}`를 발화하므로 더 이상 무음이 아니다 (QUIC은 미완결 데이터그램을 DROP해 내보내는 바이트가 없으므로 대응 계상이 없다). (b) 16 KiB 캡은 **재조립 창당** 상한이지 flow당이 아니다 — terminal 분기가 flow를 evict하므로 같은 `srcIP:sport`의 다음 세그먼트가 새 창을 연다. per-flow 추적은 도입하지 않는다(guest가 소스 포트만 바꾸면 새 flow key를 얻으므로 상태 기계가 실효 방어가 되지 않는다) |
| **TCP 삽입/재정렬 evasion**(신규 — 2026-08-06 감사 M14) | `sni.Reassembler`는 도착한 세그먼트를 **도착 순으로 단순 append**한다 — TCP 시퀀스 번호를 보지 않는다. 따라서 (a) guest가 raw 소켓으로 높은 seq 세그먼트를 먼저 보내면 anvil은 도착 순, 서버는 seq 순으로 재조립해 **서로 다른 ClientHello를 본다**(anvil이 허용 SNI를 읽고 승인 mark를 찍는 동안 서버는 비허용 도메인의 정상 ClientHello를 수신). (b) `ip_forward()`가 `ttl<=1`을 `NF_INET_FORWARD` 훅 **이전에** 검사하므로, TTL=2로 보낸 허용 SNI ClientHello는 anvil의 승인·mark를 받고 호스트 밖 첫 라우터에서 소멸하며, 뒤이은 TTL=64 비허용 ClientHello는 이미 붙은 connmark fast-path로 검사 없이 통과한다 | **봉쇄하지 않는다 — 계약으로 명시한다.** 이 클래스는 §위협 모델의 *적대적 in-guest 루트* 총론에 속하며 실제 봉쇄에는 TLS 종단이 필요하다(비목표). 다만 두 가지를 기록해 둔다: (1) **QUIC 경로는 이 클래스를 방어한다** — `internal/network/quic`의 CRYPTO 재조립기가 offset 인덱스와 `errInconsistentOverlap`으로 도착 순서 무관성을 보장한다. TCP에는 대응 방어가 없다는 **비대칭**이 설계 의도가 아니라 미구현이다. (2) 이 벡터는 표의 `SNI spoofing` 행과 다르다 — spoofing은 서버가 SNI를 무시하는 경우이고, 여기서는 **서버가 실제로 차단 대상 SNI를 보는데도 통과**하며 audit에는 허용 도메인으로 **거짓 allow 기록**이 남는다. 시나리오 (a)만 닫으려면 TCP reassembler를 seq-offset 인지로 재작성(QUIC `InitialReassembler` 수준)하면 되지만 (b)는 남는다 |

...
|---|---|---|
| **ECH/ESNI**(SNI 암호화) | anvil은 ClientHello의 **outer(공개/cover) SNI만** 관측한다(ECH `encrypted_client_hello` 확장 `0xfe0d`는 파서가 스킵). outer가 없거나 비허용 decoy면 allowlisted SNI가 없어 fail-closed DROP. **단 outer가 allowlisted 도메인(ECH 공개 이름)이면 flow는 허용되고 암호화된 inner 목적지는 은닉된다** — guest-asserted SNI와 동일 신뢰등급 잔여(허용 outer로 임의 inner 터널) | anvil은 ECH를 무력화하지 않는다(inner는 서버 키 없이 복호 불가). 완화는 해당 엔드포인트 **CIDR fallback 핀**뿐(OQ7, outer SNI는 신뢰 근거 아님). ECH 자체 차단은 확장 탐지+deny 하드닝이 필요하나 allowlisted CDN의 정상 ECH도 차단(별도 결정, 후속) |

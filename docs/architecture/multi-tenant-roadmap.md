...
- `PlacementStore`: runtime host, VM placement, snapshot location, routed flock registry
  JSON persistence
...
현재 `CheckTenantQuota`, `Scheduler.Schedule`, `QuotaStore`는 위 축에 대한
...
`POST /schedule/flock` dry-run planner는 operator planning surface를 제공한다.
...
`agent_token`을 포함하지 않는다. `GET /audit/runtime`과
`POST /audit/runtime/prune`은 조회와 보존 정책 적용을 운영 API로 제공한다.

...
- 구현 범위: 없음. 이 문서는 scheduler, quota, tenant API, egress control,
  billing, UI, daemon 동작을 구현하지 않는다.
...
향후 tenant identifier는 먼저 MCP adapter boundary에서 받아야 한다. adapter는
IronClaw 또는 상위 orchestration 계층이 제공한 tenant identifier를 검증 가능한
형태로 보존하고, daemon 계약이 확장된 뒤에만 daemon API로 전달해야 한다. daemon
계약 확장 전에는 tenant identifier를 임의 header, profile 이름, session alias,
VM ID에 끼워 넣어 quota나 audit의 근거로 사용하지 않는다.

...
- 현재 기준: `v0.2.0` single-host runtime
- 구현 범위: MCP adapter boundary의 foundation. `internal/anvilmcp/tenant_policy.go`는
  tenant ID validation, quota decision, host selection primitive, egress policy,
  runtime audit JSONL helper를 제공한다.
- 비구현 범위: scheduler service, tenant API, daemon tenant contract, host egress
  enforcement, billing, UI.
...
현재 구현된 부분은 scheduler/daemon 동작을 바꾸지 않는 library boundary다.
...
- `AppendRuntimeAudit`: symlink를 거부하는 `0600` JSONL append-only runtime audit
- `ANVIL_MCP_TENANT_ID`, `ANVIL_MCP_AUDIT_LOG`: MCP adapter의 optional
  tenant/audit 설정

이 foundation은 daemon API에 tenant header를 추가하지 않고 host network policy도
강제하지 않는다.
...
이 문서는 위 구성 요소의 책임 경계를 정의하지만 scheduler 자체를 구현하지
않는다. 현재 daemon API, MCP tool, host network 구현도 이 문서만으로 변경하지
않는다.
...
Tenant identifier는 먼저 MCP adapter boundary에서 받을 수 있다. adapter는
IronClaw 또는 상위 orchestration 계층이 제공한 `tenant_id`나
`ANVIL_MCP_TENANT_ID`를 검증 가능한 형태로 보존하고, daemon 계약이 확장된 뒤에만
daemon API로 전달해야 한다. daemon 계약 확장 전에는 tenant identifier를 임의
header, profile 이름, session alias, VM ID에 끼워 넣어 quota나 audit의 근거로
사용하지 않는다.
...
- daemon API: 계약 확장 후 VM/snapshot/task/audit operation에 tenant identifier
  수신
...
현재 `CheckTenantQuota`는 위 축에 대한 deterministic decision만 제공한다. 이
decision은 scheduler나 quota service가 호출해야 하며, 현재 daemon은 이 결과를
자동으로 강제하지 않는다.
...
현재 `SelectRuntimeHost`는 host 목록 중 healthy, capacity, requested snapshot
bytes, egress policy 조건을 만족하는 첫 host를 고르는 primitive다. 실제 host
inventory, health polling, snapshot locality, retry/failover는 아직 scheduler
service 책임으로 남아 있다.
...
현재 `EgressPolicy` 값은 policy 선택과 audit/debug를 위한 enum이다. 실제 packet
...
현재 `ANVIL_MCP_AUDIT_LOG`를 설정하면 성공한 MCP tool call에 대해 위 필드의
부분집합을 JSONL로 append한다. 이 record는 daemon raw response body, snapshot
metadata, `agent_token`을 포함하지 않는다. 실패 path, 조회 API, retention policy는
후속 작업이다.
...
현재 restore 경로의 direct token exposure는 정책이 아니라 해결해야 할 debt로
다룬다. 새로운 audit record, roadmap 예제, multi-tenant 계약은 이 노출을
정상적인 설계로 고정하지 않는다.
...
- daemon tenant contract 구현
- host egress control 구현
- billing
- UI

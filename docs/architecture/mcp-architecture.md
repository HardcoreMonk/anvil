...
- 기준 버전: upstream ephemera `v0.7.0` + anvil runtime foundation
- MCP 버전: v1 stdio adapter
- Entrypoint: `cmd/anvil-mcp`
- 런타임 대상: ephemera control plane daemon HTTP API
- 현재 확장: optional tenant/egress contract, runtime audit, Goosetown
  flock/Town Wall MCP tool surface, IronClaw/Gemini tool input schema
  compatibility 검증
...
| Schema compatibility helper | `internal/anvilmcp/ironclaw_schema.go` | anvil tool input struct에서 Gemini function declaration type이 비지 않는지 정적 검증 |
...
Goosetown flock/Town Wall tool은 기존 VM/snapshot tool을 대체하지 않는 additive
extension이다. `anvil_spawn_flock`은 `task`, `roles`, optional `tenant_id`,
optional `egress_policy`를 받아 역할 목록 기반 flock 생성을 daemon
`POST /flocks`에 위임한다. flock 계열 tool은 명시적인 `flock_id`를 사용하며,
VM convenience alias인 `session_name`을 flock alias로 재사용하지 않는다.
출력은 daemon의 flock metadata, agent 상태, Town Wall post/history response를
MCP response object로 보존하며, VM/snapshot tool처럼 daemon 의미를 adapter에서
재해석하지 않는다.
...
`internal/anvilmcp/ironclaw_schema.go`는 현재 anvil MCP tool input struct를
IronClaw/Gemini function declaration 관점의 field schema로 변환하고, tool name,
field name, Gemini type이 비어 있지 않은지 검증한다. 이 검증은 runtime behavior를
바꾸지 않고 upstream tool inventory 조합에서 빈 type declaration이 들어가는
회귀를 빠르게 잡기 위한 정적 safety check다.
...
- `internal/anvilmcp/ironclaw_schema.go`

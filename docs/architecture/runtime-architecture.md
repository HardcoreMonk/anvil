...
- 기준 버전: upstream ephemera `v0.4.3` + anvil runtime control-plane updates
...
- upstream `v0.3.2`-`v0.4.3`은 현재 sync branch baseline으로 반영되어 있다. `v0.3.2`는
  ...
  `max_agents`, Town Wall filter/rotation을 제공한다.
- 저장소/모듈 이름: `ephemera`
- 런타임 소유 파일:
  - `cmd/goose-daemon/`
  - `cmd/goose-agent/`

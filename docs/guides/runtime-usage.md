...
| **Auto-injected control-plane token** (v0.3.3; narrowed by ADR-0003) | When `EPHEMERA_API_TOKENS` is set, the host writes a control-plane bearer into each flock VM at `/root/.ephemera-cp-token` (mode 0600) and the in-VM `/townwall/post` forwarder reads it automatically — no manual `EPHEMERA_CONTROL_PLANE_TOKEN` env inside every VM. Since [ADR-0003](../adr/0003-per-flock-guest-capability-tokens.md) the injected value is that flock's **guest capability token**, which admits only `/flocks/<its own id>/(post\|wall\|wall/history\|call)` — not the operator bearer, and not any control-plane route. It has no expiry; deleting the flock is the revocation path. |
| **CP token hot rotation** (v0.3.4) | `EPHEMERA_API_TOKENS_FILE=/path/to/tokens` enables true hot rotation: edit the file, send SIGHUP, and the daemon both swaps `cp.clients` and fans the new token out over vsock (`SET_CP_TOKEN` command, atomic file rewrite inside the guest) to the VMs it injected its own bearer into. After ADR-0003 that set is only VMs spawned before it — a flock member's capability token is unrelated to operator-token rotation and is deliberately left alone. |
...
| **End-user installer** (v0.7.0, runtime/operator surface) | `install.sh` / `uninstall.sh` / `INSTALL.md` install the daemon as a systemd service (`ephemera.service.in` → canonical `ephemera` unit, `EPHEMERA_HOME=@DEST@`), and `scripts/build_release.sh` builds SLIM/FULL release tarballs. This is a **runtime/operator** installer for the ephemera daemon — not an anvil/IronClaw product wrapper; the service keeps the canonical `ephemera` name (no anvil alias). `build_release.sh` re-verifies the downloaded kernel/firecracker with `sha256sum -c` against the pins parsed from `main.go`, closing the FULL-tarball supply-chain gap that existed because the runtime skipped an artifact it already had. That runtime skip is gone as of 2026-08-06 — see **Pinned artifacts verified on every start** below. `uninstall.sh` cleans ephemera-scoped `/tmp` scratch (root-gated, prefix-anchored). Expose the daemon only behind a reverse proxy/TLS or a private network. |
...
| **Pinned artifacts verified on every start** (2026-08-06) | `EnsureKernel` and `EnsureFirecracker` used to return as soon as the file existed, so a pin only ever applied to a host that did not already have the artifact — a pin bump never reached an installed host, and a tampered artifact on disk was never noticed. Both now check what is already there. The kernel pin **is** the digest of the installed file, so it is compared directly: a match passes with no network I/O, a mismatch re-downloads, and a mismatch that cannot be re-downloaded refuses to start. The firecracker pin is the digest of the release **`.tgz`**, not of the binary extracted from it, so those two can never be equal; verification goes through a provenance sidecar `artifacts/firecracker.pin-state` holding `<tarball-sha256> <binary-sha256>`, which `build_release.sh` ships alongside the binary. That yields three outcomes rather than two. **Verified** passes offline. The other two both re-provision from the pin and differ only in what happens when that re-provisioning fails: a stamp that **disagrees** is known-bad, so a failed re-provision refuses to start, while **no stamp at all** is merely unverifiable rather than bad, so a failed re-provision keeps the existing binary with a loud error — which is what lets hosts predating the stamp and air-gapped installs still boot. |
...
| 72 | **CP token hot rotation via SIGHUP** (v0.3.4; assertions updated by ADR-0003) — restart daemon with `EPHEMERA_API_TOKENS_FILE`; spawn flock under v1; edit file to v2 + SIGHUP; verify post-rotation `/townwall/post` still 200, v1 operator bearer now 401, and the daemon log carries `msg="sighup: cp token propagated" ok=N total=M` (slog form since v0.3.5) with `ok == total`. Since [ADR-0003](../adr/0003-per-flock-guest-capability-tokens.md) the expected tally is `0/0`: the flock member holds a per-flock capability token the daemon did not mint from its operator bearer, so it is not a fan-out target, and the guest keeps working because its credential never depended on the rotated token. The line must still be emitted at `0/0` — silence would be indistinguishable from a reload that never ran. |
...
| `EPHEMERA_API_TOKENS` | *(unset)* | Per-client Bearer tokens: `alice:token1,bob:token2` (each entry may carry an optional `:expires`, see `_TOKENS_FILE` and the Token TTL docs — v0.4.1). Setting it also turns on guest control-plane credentials: with auth enabled, each flock member VM is injected its **flock's own guest capability token** at `/root/.ephemera-cp-token` so the in-VM `/townwall/post` forwarder can call back without manual setup (v0.3.3; the injected value is a per-flock token rather than a client token since [ADR-0003](../adr/0003-per-flock-guest-capability-tokens.md)). None of the tokens listed here is injected into a VM. v0.3.4 SIGHUP fan-out rotates only VMs the daemon injected its own bearer into — after ADR-0003, only VMs spawned before it. See `_TOKENS_FILE` for true hot rotation. |
...
`EPHEMERA_API_ADDR` takes precedence over `EPHEMERA_API_PORT`. Most variables are read at startup; use SIGHUP to reload tokens. With `EPHEMERA_API_TOKENS_FILE` SIGHUP also propagates the first non-expired client's token, via vsock, to the VMs whose `/root/.ephemera-cp-token` the daemon populated with that same operator bearer (v0.3.4; first-non-expired since v0.4.1). Since [ADR-0003](../adr/0003-per-flock-guest-capability-tokens.md) no spawn path marks a VM that way, so in steady state that set is empty and the fan-out logs `ok=0 total=0`; flock members hold a per-flock capability token that operator-token rotation neither touches nor invalidates.

...
- `spawnVMForFlock` (plus `restartAgent` and `changeFlockAgentRole`) pass the token through `spawnVMOptions.ControlPlaneToken` → `VMPrepareOptions.ControlPlaneToken` → `injectVMFiles`, which writes it to `/root/.ephemera-cp-token` at mode 0600. Because these are the only call sites that source the token from `controlPlaneTokenForVM()`, they are also the only ones that set `spawnVMOptions.ControlPlaneTokenManaged` — the provenance bit that makes a VM eligible for SIGHUP rotation (mirrored to `runningVM.cpTokenManaged` and persisted as `VMState.CPTokenManaged`).
...
- A `POST /vms` that *does* carry `control_plane_token` (how a routed flock member is spawned — the value is that flock's scoped relay token, not the operator bearer) injects the caller's token verbatim and leaves it unmanaged: it is the caller's credential, and the home daemon expects to keep seeing exactly it.
...
| Local flock member (`spawnVMForFlock`, `restartAgent`, `changeFlockAgentRole`) | `controlPlaneTokenForVM()` — the daemon's own operator bearer | **Rotated** |
| Routed flock member (`POST /vms` with `control_plane_token`) | Caller-supplied per-flock **scoped relay token** | **Skipped** |

The rule is recorded explicitly at spawn time rather than inferred from `flock_id` or flock kind, because a routed flock member also carries a `flock_id` yet must keep its narrower relay token: the daemon rotates only the credential it owns. The bit is persisted in `state.json` as `cp_token_managed` so rotation still reaches local flock members after a daemon restart. A `state.json` written before that field existed decodes as unmanaged; use the `POST /flocks/{id}/agents/{agent_id}/restart` fallback below to re-inject the current token and re-mark such a VM.

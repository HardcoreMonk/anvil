...
anvil 0.2.0은 단일 host에서 VM 생성, task 실행, agent proxy, VM별 token,
snapshot/restore 기반을 제공한다. IronClaw 통합 v1의 목표는 이 runtime
capability를 MCP tool로 노출해 IronClaw가 anvil VM lifecycle과 task 실행을
사용할 수 있게 하는 것이다.

v1은 **thin runtime bridge**로 설계한다. MCP adapter는 anvil daemon API를 얇게
...
v1은 anvil daemon 0.2.0 API를 Go 기반 MCP stdio server로 얇게 감싸는 통합이다.

...
anvil 0.2.0은 격리된 AI 작업 실행 환경을 단일 서버에서 더 안정적으로 운영하기 위한 릴리즈다. 0.1.0이 "격리된 VM을 만들고 작업을 실행할 수 있음"을 증명한 단계였다면, 0.2.0은 "그 VM의 상태를 저장하고, 다시 복원하고, 외부 시스템이 더 안전하게 사용할 수 있음"으로 발전했다.
...
0.2.0은 anvil을 "AI coding agent용 격리 실행 환경"으로 만들기 위한 중요한 전환점이다.
...
anvil 0.2.0은 0.1.0보다 훨씬 제품에 가까워졌다. 가장 큰 변화는 snapshot과 restore다. 이제 anvil은 단순히 격리 VM을 띄우는 시스템이 아니라, AI 작업 세션의 상태를 저장하고 다시 불러올 수 있는 실행 기반으로 발전했다.

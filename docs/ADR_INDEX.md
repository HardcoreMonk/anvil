...
| `v0.4.2` | adapted, default cow deferred(flip 승인 대기) | COW probe/fallback과 COW+Diff snapshot은 채택한다. `EPHEMERA_DISK_MODE=cow`는 anvil에서 명시적 opt-in이다. default 전환 burn-in 조건은 충족됐다 — run 1(host-a full cow gate)이 diff-restore `500`으로 FAIL→D4 회부→D4 fix(PR #46: `overlaySparseDiff`가 loop attach 전 `out.Sync()`) 후 재-burn-in `334✓/0✗` green. 실제 default flip은 별도 승인 slice로 대기(run 기록 [`operations/2026-07-11-cow-burnin-run.md`](operations/2026-07-11-cow-burnin-run.md)). |
...
default COW 전환은 burn-in 조건이 충족됐다(run 1 FAIL→D4 fix(PR #46) 후 재-burn-in
`334✓/0✗` green) — 실제 default flip만 별도 승인 대기다. 남은 비목표는 runtime MCP
